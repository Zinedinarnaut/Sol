...
fragment float4 fragment_main(
    ...
    // The fullscreen-triangle UVs interpolate to 0...1 across the drawable.
    // Halving them here sampled only one quarter of the artwork, which made
    // backgrounds look zoomed, cropped, and dominated by a single colour.
    float2 uv = in.uv;
    ...
    float2 motion = float2(scroll, 0.0);
    ...
    if (u.hasBackground > 0.5) {
        ...
        if (u.transition < 0.999) {
            ...
            baseColor = mix(prevCover, currCover, u.transition);
        } else {
            baseColor = currCover;
        }
    } else {
        float gradient = 0.12 + 0.06 * (1.0 - uv.y);
        float noise = hash21(uv * float2(120.0, 80.0));
        float softNoise = mix(noise, hash21(uv * float2(40.0, 22.0)), 0.6);
    ...
    baseColor.rgb = mix(baseColor.rgb, float3(luma), 0.28);
    baseColor.rgb *= 0.70;

    float vignette = smoothstep(1.0, 0.2, length(centered));
    baseColor.rgb *= mix(0.72, 1.0, vignette);
