...
namespace Ryujinx.Graphics.Vulkan
{
    public unsafe static class VulkanInitialization
    {
        ...
        internal static VulkanInstance CreateInstance(Vk api, GraphicsDebugLevel logLevel, string[] requiredExtensions)
        {
            ...
            ExportMetalObjectCreateInfoEXT exportMetalCommandQueue = new()
            {
                SType = StructureType.ExportMetalObjectCreateInfoExt,
                ExportObjectType = (ExportMetalObjectTypeFlagsEXT)2,
            };

            if (OperatingSystem.IsMacOS())
            {
                instanceCreateInfo.PNext = &exportMetalCommandQueue;
            }
